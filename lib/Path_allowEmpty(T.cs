...
namespace nilnul.relation
{
	...
	/// there may be cycles.
	/// </remarks>
	public class Path_allowEmpty<T>
	{
...
IEqualityComparer<T> eqComparer
		...
		public Path_allowEmpty(IEqualityComparer<T> eq)
		...
		public Path_allowEmpty(Pair_withNoEq.Pair<T> pair, IEqualityComparer<T> eq)
			//: this(new[] { pair }, eq)
		...
		static public bool IsEmpty(Path_allowEmpty<T> path)
		{
		...
		static public bool IsNotEmpty(Path_allowEmpty<T> path)
		{
			return !IsEmpty(path);
		...
		public class IsEmptySingleton
			: nilnul.bit.Be<Path_allowEmpty<T>>
		{
			...
			: nilnul.bit.Be<Path_allowEmpty<T>>
		{
			...
			public class Asserted :
				nilnul.bit.be.Asserted<Path_allowEmpty<T>, IsNotEmptySingleton>
			{

				static public T Start(Path_allowEmpty<T> path_notEmpty)
				{
					return path_notEmpty.pairs.First().first;

				}

				static public T End(Path_allowEmpty<T> path_notEmpty)
				{
					return path_notEmpty.pairs.Last().second;

				}

				public Asserted(Path_allowEmpty<T> path)
					: base(path)
				{
			...
			where TEq : IEqualityComparer<T>, new()
		{
			...
			static public Path_withEq<TEq> Concat(Path_withEq<TEq> a, Path_withEq<TEq> b)
			{
				...
				return new Path_withEq<TEq>(Path_allowEmpty<T>.Op.Concat(a.pairs, b.pairs, ElementEq));
			}
