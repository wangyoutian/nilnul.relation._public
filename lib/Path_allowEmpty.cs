...
namespace nilnul.relation
{
	...
	public partial class Path_allowEmpty
	{
		/// <summary>
		/// 
		/// </summary>
...
IEqualityComparer<T> eqComparer
			...
			public Path(IEqualityComparer<T> eq)
			...
			public Path(Pair_withNoEq.Pair<T> pair, IEqualityComparer<T> eq)
				//: this(new[] { pair }, eq)
			...
			static public bool IsEmpty(Path<T> path)
			{
			...
			static public bool IsNotEmpty(Path<T> path)
			{
				return !IsEmpty(path);
				...
				: nilnul.bit.Be<Path<T>>
			{
				...
				public class Asserted :
					nilnul.bit.be.Asserted<Path<T>, IsNotEmptySingleton>
				{

					static public T Start(Path<T> path_notEmpty)
					{
						return path_notEmpty.pairs.First().first;

					}

					static public T End(Path<T> path_notEmpty)
					{
						return path_notEmpty.pairs.Last().second;

					}

					public Asserted(Path<T> path)
						: base(path)
					{
				...
				where TEq : IEqualityComparer<T>, new()
			{
				...
				static public Path_withEq<TEq> Concat(Path_withEq<TEq> a, Path_withEq<TEq> b) {
					...
					return new Path_withEq<TEq>( Path<T>.Op.Concat(a.pairs, b.pairs,ElementEq));
				}
		...
		public class Path_withStaticElementEq<T,TEq>
